...
const Bulb = ({ isOn, setIsOn }) => {
  ...
  useEffect(() => {
    document.body.className = isOn ? "dark" : "light";
  }, [isOn]);
  ...
  useEffect(() => {
    ...
    const DUMMY_CORD = svgRef.current.querySelector(
      ".toggle-scene__dummy-cord line"
    );
    const PROXY = proxyRef.current;

    if (!CORDS || !HIT || !DUMMY || !DUMMY_CORD) return;
    ...
    // Create timeline for cord animation
    const CORD_TL = gsap.timeline({
      ...
      onStart: () => {
        ...
        AUDIO.CLICK.play().catch(() => {
          // Audio play failed - this is okay, user interaction might be required
        });
      },
      onComplete: () => {
        gsap.set([DUMMY, HIT], {
    ...
    cordTlRef.current = CORD_TL;

    // Create draggable interaction
  ...
  return (
    <>
      <div ref={proxyRef} className="proxy" />
